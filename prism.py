try:
    ...
    try:
        from prism_customization import default_colors # prism_customization file
    except ImportError:
        default_colors = {}
...
TIMEOUT_YDLP: int = 60 #script will wait for given amount of time before retrying the next video
CSV_FILENAME = "prism_results.csv"
LOGO_TYPE = default_colors.get("dc_theme_default")
...
def catch_all_videos(channel: str) -> list[str]:
    videos: list[str] = []
...
def write_csv(videos: list[str], timeout: int=15) -> None:
    with YoutubeDL(ydl_opts) as ydl:
        with open(CSV_FILENAME, "w", newline="", encoding="utf-8") as file:
            ...
            for index, url in enumerate(videos, start=1):
                ...
                try:
                    ...
                    print(f"{url} failed. You may retry")
                    print(exp)
                    time.sleep(TIMEOUT_YDLP)
...
def start() -> None:
    print("Reuse existing data (y)")
    use_existing: str = input(">>> ")

    if use_existing.lower().strip() == "y":
        displaycsv()

    else:
        print("Channel to analyze")
        channel_to: str = input(">>> ")
        print("Timeout")
        while True:
            try:
                timeout = int(input(">>> "))
        ...
        videos: list[str] = catch_all_videos(channel_to)
        write_csv(videos, timeout)
        displaycsv()
        print("Finished!")
